...
def get_features(post, thread, data_features):
	...
	number_posts=normalize(number_posts)
	data_features=np.concatenate([number_posts.reshape(-1,1), numb_unique.as_matrix().reshape(-1,1),
                                     numb_quotes.reshape(-1,1),q_perc.reshape(-1,1),
                                     avg_numb_of_posts.as_matrix().reshape(-1,1),data_features],axis=1)
